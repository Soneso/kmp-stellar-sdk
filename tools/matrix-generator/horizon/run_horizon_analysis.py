...
3. Analyzes the KMP Stellar SDK implementation
...
from sdk_analyzer import KmpSDKAnalyzer
...
class HorizonAnalysisPipeline:
    ...
    def __init__(
        ...
        # Define paths using common module constants
        self.data_dir = DATA_DIR / "horizon"
        self.horizon_dir = COMPATIBILITY_DIR / "horizon"
        ...
        self.sdk_implementation_file = self.data_dir / "kmp_sdk_implementation.json"
    ...
    def run(self) -> int:
        ...
        try:
            ...
            # Step 3: Analyze KMP SDK
            self.analyze_kmp_sdk()
    ...
    def fetch_horizon_release(self) -> None:
        ...
        if self.local_router_path:
            ...
            self.progress.finish_step(
                f"Loaded {len(self.router_source)} bytes from local file"
            )
    ...
    def parse_horizon_endpoints(self) -> None:
        ...
        if not self.router_source:
            raise RuntimeError(
                "No router source available. Step 1 must complete first."
            )

        # Parse router source directly from content (no temp file needed)
        parser = HorizonRouterParser()
        parser.parse_from_content(self.router_source, version_info=self.release_info)
    ...
    def analyze_kmp_sdk(self) -> None:
        """Step 3: Analyze KMP SDK implementation."""
        self.progress.start_step("Analyzing KMP SDK")

        # Analyze SDK (KmpSDKAnalyzer uses SDK_ROOT from common module)
        analyzer = KmpSDKAnalyzer()
        analyzer.analyze()

        # Save results
        analyzer.save_json(self.sdk_implementation_file)
        ...
        self.progress.finish_step(
            f"Found {builders_count} request builders "
            f"({exposed_count} exposed), SDK v{sdk_version}"
        )
    ...
    def collect_statistics(self) -> Dict[str, Any]:
        ...
        generated_files = [
            str(self.horizon_endpoints_file.relative_to(SDK_ROOT)),
            str(self.sdk_implementation_file.relative_to(SDK_ROOT)),
            str(self.comparison_file.relative_to(SDK_ROOT)),
            str(self.statistics_file.relative_to(SDK_ROOT)),
            str(self.markdown_file.relative_to(SDK_ROOT)),
        ]

        return {
            "horizon_version": self.release_info["version"],
            "sdk_version": stats.get("sdk_version", get_sdk_version()),
