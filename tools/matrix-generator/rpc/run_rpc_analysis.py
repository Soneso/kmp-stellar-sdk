...
3. Analyzes the KMP Stellar SDK Soroban implementation
...
class RPCAnalysisPipeline:
    ...
    def __init__(
        ...
        self.data_dir = DATA_DIR / "rpc"
        self.rpc_dir = COMPATIBILITY_DIR / "rpc"
        ...
        self.sdk_implementation_file = self.data_dir / "kmp_soroban_implementation.json"
        ...
        if self.local_jsonrpc_path:
            ...
            if not local_path.exists():
                raise FileNotFoundError(
                    f"Local jsonrpc.go not found: {local_path}"
                )
            ...
            self.progress.finish_step(
                f"Loaded {len(self.jsonrpc_source)} bytes from local file"
            )
    ...
    def parse_rpc_methods(self) -> None:
        ...
        self.progress.finish_step(
            f"Parsed {methods_count} RPC methods"
        )

    def analyze_kmp_sdk(self) -> None:
        """Step 3: Analyze KMP SDK Soroban implementation."""
        ...
        if not soroban_server_path.exists():
            raise FileNotFoundError(
                f"Soroban server file not found: {soroban_server_path}"
            )

        # Analyze SDK
        analyzer = SorobanSDKAnalyzer(str(soroban_server_path))
        kmp_data = analyzer.analyze()

        # Save results
        with open(self.sdk_implementation_file, "w", encoding="utf-8") as f:
            json.dump(kmp_data, f, indent=2, ensure_ascii=False)

        methods_count = kmp_data["metadata"]["total_methods"]
        sdk_version = get_sdk_version()
        self.progress.finish_step(
            f"Found {methods_count} Soroban methods in SorobanServer.kt, "
            f"SDK v{sdk_version}"
        )
    ...
    def collect_statistics(self) -> Dict[str, Any]:
        ...
        # Collect file paths relative to SDK root
        generated_files = [
            str(self.rpc_methods_file.relative_to(SDK_ROOT)),
