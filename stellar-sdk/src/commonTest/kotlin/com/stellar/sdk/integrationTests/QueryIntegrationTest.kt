...
import com.stellar.sdk.horizon.responses.*
import com.stellar.sdk.horizon.responses.effects.AccountCreatedEffectResponse
...
class QueryIntegrationTest {
     ...
     */
    private fun assetToTriple(asset: com.stellar.sdk.Asset): Triple<String, String?, String?> {
        return when (asset) {
     ...
     */
    private fun assetsToTriples(assets: List<com.stellar.sdk.Asset>): List<Triple<String, String?, String?>> {
     ...
     */
    @Test
    fun testQueryAccounts() = runBlocking {
        withTimeout(120_000) {
            ...
            val accountsForSignerLimited = horizonServer.accounts()
                .forSigner(accountId)
                .limit(2)
                .order(com.stellar.sdk.horizon.requests.RequestBuilder.Order.DESC)
                .execute()
     ...
     */
    @Test
    fun testQueryAssets() = runBlocking {
        withTimeout(60_000) {
     ...
     */
    @Test
    fun testQueryEffects() = runBlocking {
        withTimeout(60_000) {
            ...
            val effectsPage = horizonServer.effects()
                ...
                .limit(3)
                .order(com.stellar.sdk.horizon.requests.RequestBuilder.Order.ASC)
                .execute()
     ...
     */
    @Test
    fun testQueryOperationsForClaimableBalance() = runBlocking {
        withTimeout(60_000) {
     ...
     */
    @Test
    fun testQueryTransactionsForClaimableBalance() = runBlocking {
        withTimeout(60_000) {
     ...
     */
    @Test
    fun testQueryLedgers() = runBlocking {
        withTimeout(60_000) {
            // Query latest ledger
            val ledgersPage = horizonServer.ledgers()
                .limit(1)
     ...
     */
    @Test
    fun testQueryFeeStats() = runBlocking {
        withTimeout(60_000) {
     ...
     */
    @Test
    fun testQueryOffersAndOrderBook() = runBlocking {
        withTimeout(120_000) {
            ...
            transaction = TransactionBuilder(
                ...
                .addOperation(
                    ManageBuyOfferOperation(
                        ...
                        price = com.stellar.sdk.Price.fromString(price)
                    )
                )
                .setTimeout(TransactionPreconditions.TIMEOUT_INFINITE)
                .setBaseFee(AbstractTransaction.MIN_BASE_FEE)
     ...
     */
    @Test
    fun testQueryStrictSendReceivePathsAndTrades() = runBlocking {
        withTimeout(180_000) {
            ...
            transaction = TransactionBuilder(
                ...
                .addOperation(
                    ManageSellOfferOperation(
                        ...
                        buying = iomAsset,
                        amount = "100",
                        price = com.stellar.sdk.Price.fromString("0.5")
                    )
                )
                .setTimeout(TransactionPreconditions.TIMEOUT_INFINITE)
                .setBaseFee(AbstractTransaction.MIN_BASE_FEE)
            ...
            val stream = horizonServer.trades()
                ...
                .stream(
                    ...
                    listener = object : com.stellar.sdk.horizon.requests.EventListener<TradeResponse> {
