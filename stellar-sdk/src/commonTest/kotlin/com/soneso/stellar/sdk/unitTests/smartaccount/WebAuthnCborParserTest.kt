...
class WebAuthnCborParserTest {
    ...
    /**
     * Encodes a CBOR unsigned integer (major type 0).
     */
    private fun buildCborUnsignedInt(value: Int): ByteArray {
        return buildCborHead(0, value)
    }

    /**
     * Builds a CBOR map with text-string keys and byte-string values.
     */
    private fun buildCborMap(entries: List<Pair<String, ByteArray>>): ByteArray {
        var result = buildCborHead(5, entries.size)
        for ((key, value) in entries) {
            result += buildCborTextString(key)
            result += buildCborByteString(value)
        }
        return result
    }
