...
class ContextRuleParsingTest {
    ...
    @Test
    fun testParseContextRule_signerIdsNotVec_throwsValidationException() = runTest {
        val kit = OZSmartAccountKit.create(buildConfig())
        val manager = kit.contextRuleManager
        ...
        assertTrue(ex.message!!.contains("signer_ids"), "Exception message should mention 'signer_ids'")
        assertTrue(ex.message!!.contains("Vec"), "Exception message should state that a Vec was expected")
    }

    @Test
    fun testParseContextRule_policiesNotVec_throwsValidationException() = runTest {
        val kit = OZSmartAccountKit.create(buildConfig())
        val manager = kit.contextRuleManager
        ...
        val ruleMap = buildMapScVal(listOf(
            ...
            "name" to Scv.toString("TestRule"),
            "context_type" to defaultContextTypeScVal(),
            "policies" to addressScVal(validContractAddress2)
        ))
    ...
    fun testParseContextRule_voidInput_throwsValidationException() = runTest {
        ...
        assertTrue(
            ex.message.contains("Expected Map ScVal for context rule"),
            "A non-Map ScVal must be rejected by the map guard; got: ${ex.message}"
        )
    }
    ...
    fun testParseContextRule_stringInput_throwsValidationException() = runTest {
        ...
        assertTrue(
            ex.message.contains("Expected Map ScVal for context rule"),
            "A String ScVal must be rejected by the map guard; got: ${ex.message}"
        )
    }
    ...
    fun testParseContextRule_unknownContextTypeDiscriminant_throwsValidationException() = runTest {
        ...
        // The discriminant must not be the literal word "Unknown": the guard's own message
        // template starts with "Unknown context_type discriminant:", so a fixture named
        // "Unknown" would satisfy the assertion even if the discriminant were never
        // interpolated — exactly the regression this test exists to catch.
        val unknownType = Scv.toVec(listOf(Scv.toSymbol("Bogus")))
        val ruleMap = buildFullRuleMap(contextType = unknownType)

        val ex = assertFailsWith<ValidationException.InvalidInput> {
            manager.parseContextRule(ruleMap)
        }
        assertTrue(ex.message.contains("Bogus"), "Exception message must echo the unknown discriminant; got: ${ex.message}")
        assertTrue(ex.message.contains("context_type"), "Exception message should mention 'context_type'")
    }
    ...
    @Test
    fun testParseContextRule_callContractElementNotAddress_throwsValidationException() = runTest {
        val kit = OZSmartAccountKit.create(buildConfig())
        val manager = kit.contextRuleManager

        // CallContract carrying a String where the contract address belongs
        val badCallContract = Scv.toVec(listOf(
            Scv.toSymbol("CallContract"),
            Scv.toString(validContractAddress2)
        ))
        val ruleMap = buildFullRuleMap(contextType = badCallContract)
        ...
        assertTrue(ex.message!!.contains("CallContract"), "Exception message should mention 'CallContract'")
        assertTrue(ex.message!!.contains("Address"), "Exception message should state that an Address was expected")
    }

    @Test
    fun testParseContextRule_createContractElementNotBytes_throwsValidationException() = runTest {
        val kit = OZSmartAccountKit.create(buildConfig())
        val manager = kit.contextRuleManager

        // CreateContract carrying a U32 where the WASM hash bytes belong
        val badCreateContract = Scv.toVec(listOf(
            Scv.toSymbol("CreateContract"),
            Scv.toUint32(7u)
        ))
        val ruleMap = buildFullRuleMap(contextType = badCreateContract)
        ...
        assertTrue(ex.message!!.contains("CreateContract"), "Exception message should mention 'CreateContract'")
        assertTrue(ex.message!!.contains("Bytes"), "Exception message should state that Bytes were expected")
    }
    ...
    @Test
    fun testParseContextRule_signerDiscriminantNotSymbol_throwsValidationException() = runTest {
        val kit = OZSmartAccountKit.create(buildConfig())
        val manager = kit.contextRuleManager

        val badSigner = Scv.toVec(listOf(
            Scv.toUint32(0u),
            Scv.toAddress(Address(validAccountAddress).toSCAddress())
        ))
        val ruleMap = buildFullRuleMap(
            signers = listOf(badSigner),
            signerIds = listOf(1u)
        )
        ...
        assertTrue(ex.message!!.contains("signer"), "Exception message should mention 'signer'")
        assertTrue(ex.message!!.contains("Symbol"), "Exception message should state that a Symbol was expected")
    }

    @Test
    fun testParseContextRule_delegatedSignerElementNotAddress_throwsValidationException() = runTest {
        val kit = OZSmartAccountKit.create(buildConfig())
        val manager = kit.contextRuleManager

        val badDelegated = Scv.toVec(listOf(
            Scv.toSymbol("Delegated"),
            Scv.toString(validAccountAddress)
        ))
        val ruleMap = buildFullRuleMap(
            signers = listOf(badDelegated),
            signerIds = listOf(1u)
    ...
    @Test
    fun testParseContextRule_externalSignerVerifierNotAddress_throwsValidationException() = runTest {
        val kit = OZSmartAccountKit.create(buildConfig())
        val manager = kit.contextRuleManager
        ...
        assertTrue(ex.message!!.contains("verifier"), "Exception message should mention the verifier element")
        assertTrue(ex.message!!.contains("Address"), "Exception message should state that an Address was expected")
    }

    @Test
    fun testParseContextRule_externalSignerKeyDataNotBytes_throwsValidationException() = runTest {
        val kit = OZSmartAccountKit.create(buildConfig())
        val manager = kit.contextRuleManager

        val badExternal = Scv.toVec(listOf(
            Scv.toSymbol("External"),
            Scv.toAddress(Address(validContractAddress2).toSCAddress()),
            Scv.toString("not-bytes")
        ))
        val ruleMap = buildFullRuleMap(
            signers = listOf(badExternal),
            signerIds = listOf(1u)
        ...
        assertTrue(ex.message!!.contains("keyData"), "Exception message should mention 'keyData'")
        assertTrue(ex.message!!.contains("Bytes"), "Exception message should state that Bytes were expected")
    }
    ...
    fun testContextRuleType_callContractInvalidAddress_throwsValidationException() {
        ...
        assertTrue(
            ex.message.contains("INVALID_ADDRESS"),
            "Exception message must echo the rejected address; got: ${ex.message}"
        )
    }
    ...
    @Test
    fun testContextRuleType_createContractSameInstanceIsEqual() {
        val rule = ContextRuleType.CreateContract(ByteArray(32) { it.toByte() })
        assertEquals(rule, rule, "A CreateContract rule type must equal itself")
    }

    @Test
    fun testContextRuleType_createContractDifferentWasmHash_notEqual() {
        val a = ContextRuleType.CreateContract(ByteArray(32) { it.toByte() })
        val b = ContextRuleType.CreateContract(ByteArray(32) { (it + 1).toByte() })
        assertNotEquals(a, b, "Differing WASM hashes must not compare equal")
    }

    @Test
    fun testContextRuleType_createContractShorterWasmHash_notEqual() {
        val a = ContextRuleType.CreateContract(ByteArray(32) { it.toByte() })
        val b = ContextRuleType.CreateContract(ByteArray(16) { it.toByte() })
        assertNotEquals(a, b, "A truncated WASM hash must not compare equal to the full hash")
    }

    @Test
    fun testContextRuleType_createContractComparedToNullAndOtherType_notEqual() {
        val rule = ContextRuleType.CreateContract(ByteArray(32) { it.toByte() })
        assertFalse(rule.equals(null), "A CreateContract rule type must not equal null")
        assertFalse(
            rule.equals(ContextRuleType.CallContract(validContractAddress)),
            "A CreateContract rule type must not equal a CallContract rule type"
        )
    }
    ...
    fun testAddContextRule_tooManySigners_throwsValidationException() = runTest {
        ...
        assertTrue(
            ex.message.contains("more than 15 signers"),
            "Exception must name the MAX_SIGNERS limit; got: ${ex.message}"
        )
    }
    ...
    fun testAddContextRule_tooManyPolicies_throwsValidationException() = runTest {
        ...
        // A bare contains("5") would also be satisfied by a policy address that happens to
        // contain the character '5', so the limit must be named in full.
        assertTrue(
            ex.message.contains("more than 5 policies"),
            "Exception must name the MAX_POLICIES limit; got: ${ex.message}"
        )
    }
    ...
    fun testAddContextRule_gAddressAsPolicy_throwsValidationException() = runTest {
        ...
        assertTrue(
            ex.message.contains("policyAddress must be a valid contract address"),
            "The policy key must be rejected as a non-contract address; got: ${ex.message}"
        )
        assertTrue(
            ex.message.contains(validAccountAddress),
            "Exception message must echo the rejected G-address; got: ${ex.message}"
        )
    }
