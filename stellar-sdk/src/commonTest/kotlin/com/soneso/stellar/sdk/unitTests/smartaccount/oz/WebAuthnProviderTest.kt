...
class WebAuthnProviderTest {
    ...
    fun testNormalizeSignature_sWithLeadingZeroPadding() {
        ...
        // s = 0x8080..80 exceeds the P-256 half-order, so the padding must be stripped AND
        // low-S normalization must map it to n - s, where
        // n = ffffffff00000000ffffffffffffffffbce6faada7179e84f3b9cac2fc632551.
        val expectedS = hexToByteArray(
            "7f7f7f7e7f7f7f807f7f7f7f7f7f7f7f3c667a2d26971e0473394a427be2a4d1"
        )
        assertContentEquals(
            expectedS,
            compact.copyOfRange(32, 64),
            "High-S with DER leading-zero padding must be stripped and normalized to n - s"
        )
    }
    ...
    @Test
    fun testRegistrationResult_authenticatorMetadataParticipatesInEquality() {
        val credId = ByteArray(16) { it.toByte() }
        val attestObj = buildAttestationObject()
        ...
        val baseline = result(listOf("internal"), "multiDevice", true)

        assertEquals(
            baseline,
            result(listOf("internal"), "multiDevice", true),
            "Results carrying identical metadata must be equal"
        )
        assertNotEquals(
            baseline,
            result(listOf("hybrid"), "multiDevice", true),
            "Differing transports must break equality"
        )
        assertNotEquals(
            baseline,
            result(null, "multiDevice", true),
            "Absent transports must not equal present transports"
        )
        assertNotEquals(
            baseline,
            result(listOf("internal"), "singleDevice", true),
            "Differing deviceType must break equality"
        )
        assertNotEquals(
            baseline,
            result(listOf("internal"), null, true),
            "Absent deviceType must not equal present deviceType"
        )
        assertNotEquals(
            baseline,
            result(listOf("internal"), "multiDevice", false),
            "Differing backedUp must break equality"
        )
        assertNotEquals(
            baseline,
            result(listOf("internal"), "multiDevice", null),
            "Absent backedUp must not equal present backedUp"
        )
    }

    @Test
    fun testRegistrationResult_authenticatorMetadataParticipatesInHashCode() {
        val credId = ByteArray(16) { it.toByte() }
        val attestObj = buildAttestationObject()
        ...
        val allAbsent = result(null, null, null)

        assertEquals(
            allAbsent.hashCode(),
            result(null, null, null).hashCode(),
            "Equal results must share a hash code"
        )
        assertNotEquals(
            allAbsent.hashCode(),
            result(listOf("internal"), null, null).hashCode(),
            "transports must contribute to the hash code"
        )
        assertNotEquals(
            allAbsent.hashCode(),
            result(null, "multiDevice", null).hashCode(),
            "deviceType must contribute to the hash code"
        )
        assertNotEquals(
            allAbsent.hashCode(),
            result(null, null, true).hashCode(),
            "backedUp must contribute to the hash code"
        )
    }

    @Test
    fun testRegistrationResult_equalsRejectsNullAndForeignTypes() {
        val result = WebAuthnRegistrationResult(
            credentialId = ByteArray(16) { it.toByte() },
            publicKey = expectedPublicKey,
            attestationObject = buildAttestationObject()
        )
        val foreign: Any = WebAuthnAuthenticationResult(
            credentialId = ByteArray(16) { it.toByte() },
        ...
        assertTrue(result.equals(result), "A registration result must equal itself")
        assertFalse(result.equals(null), "A registration result must never equal null")
        assertFalse(
            result.equals(foreign),
            "A registration result must never equal an instance of another type"
        )
    }
    ...
    @Test
    fun testAuthenticationResult_equalsRejectsNullAndForeignTypes() {
        val result = WebAuthnAuthenticationResult(
            credentialId = ByteArray(16) { it.toByte() },
            authenticatorData = ByteArray(37) { it.toByte() },
            clientDataJSON = ByteArray(8) { it.toByte() },
            signature = ByteArray(64) { it.toByte() }
        )
        val foreign: Any = WebAuthnRegistrationResult(
            credentialId = ByteArray(16) { it.toByte() },
            publicKey = expectedPublicKey,
            attestationObject = buildAttestationObject()
        )

        assertTrue(result.equals(result), "An authentication result must equal itself")
        assertFalse(result.equals(null), "An authentication result must never equal null")
        assertFalse(
            result.equals(foreign),
            "An authentication result must never equal an instance of another type"
        )
    }

    // =========================================================================
    // AllowCredential Data Class Tests
    // =========================================================================

    @Test
    fun testAllowCredential_equalsRejectsNullAndForeignTypes() {
        val credential = AllowCredential(
            id = ByteArray(16) { it.toByte() },
            transports = listOf("internal")
        )
        val foreign: Any = listOf("internal")

        assertTrue(credential.equals(credential), "An allow credential must equal itself")
        assertFalse(credential.equals(null), "An allow credential must never equal null")
        assertFalse(
            credential.equals(foreign),
            "An allow credential must never equal an instance of another type"
        )
    }
