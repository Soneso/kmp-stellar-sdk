...
class RequestBuilderExecuteTest {
    ...
    @Test
    fun testExecuteGet_tooManyRequests429_throwsTooManyRequestsWithBody() = runTest {
        val server = HorizonServer(
            TEST_SERVER_URL,
            httpClient = createStatusMockClient("rate limit exceeded", HttpStatusCode.TooManyRequests)
        )
        val exception = assertFailsWith<TooManyRequestsException> {
            server.accounts().execute()
        }
        assertEquals(429, exception.code)
        assertEquals("rate limit exceeded", exception.body)
        server.close()
    }

    @Test
    fun testExecuteGet_notFound404_throwsBadRequestWithBody() = runTest {
        val server = HorizonServer(
            TEST_SERVER_URL,
            httpClient = createStatusMockClient("resource missing", HttpStatusCode.NotFound)
        )
        val exception = assertFailsWith<BadRequestException> {
            server.accounts().execute()
        }
        assertEquals(404, exception.code)
        assertEquals("resource missing", exception.body)
        server.close()
    }
