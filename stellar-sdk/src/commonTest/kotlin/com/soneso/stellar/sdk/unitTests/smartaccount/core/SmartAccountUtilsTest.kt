...
class SmartAccountUtilsTest {
    ...
    // Second known-good P-256 point: public key Q from the ECDSA test vector in
    // RFC 6979 section A.2.5 (private key d = C9AFA9D8...120F6721).
    private val rfc6979PublicKeyX = hexToBytes(
        "60fed4ba255a9d31c961eb74c6356d68c049b8923b61fa6ce669622e60f29fb6"
    )
    private val rfc6979PublicKeyY = hexToBytes(
    ...
    fun parseDerSignature_valid32ByteComponents() {
        ...
        assertEquals(bigIntFromHex("01".repeat(32)), rBig)
        assertEquals(bigIntFromHex("02".repeat(32)), sBig)
    }
    ...
    fun parseDerSignature_stripsLeadingZeroPadding() {
        ...
        val (rBig, sBig) = SmartAccountUtils.parseDerSignature(der)
        // R is the component whose padding stripping this test exists to prove.
        assertEquals(bigIntFromHex("80" + "01".repeat(31)), rBig)
        assertEquals("1", sBig.toString(10))
    }

    @Test
    fun parseDerSignature_validStandard() {
        // 32-byte R and S with the high bit clear, so no DER padding is involved
        ...
        val (rBig, sBig) = SmartAccountUtils.parseDerSignature(buildDer(rBytes, sBytes))
        assertEquals(
            bigIntFromHex("1c7ab8e46f91d3f9dbff8c50a37a0d13bbf835ac31c5d0da2dbf1e8a91c10521"),
            rBig
        )
        assertEquals(
            bigIntFromHex("2a3b4c5d6e7f8091a2b3c4d5e6f70819283746556677889900aabbccddeeff01"),
            sBig
        )
    }

    @Test
    fun parseDerSignature_leadingZeroStrippedFromR() {
        // R carries the 0x00 byte DER prepends when the high bit is set; the value is unchanged
        val rBytes = hexToBytes("b23694f0367f3e621a8458fc24d1dce654be3e2e2c1bacea40cd7a5e7a134540")
        val sBytes = hexToBytes("1c7ab8e46f91d3f9dbff8c50a37a0d13bbf835ac31c5d0da2dbf1e8a91c10521")
        val der = buildDer(byteArrayOf(0x00) + rBytes, sBytes)
        val (rBig, sBig) = SmartAccountUtils.parseDerSignature(der)
        assertEquals(
            bigIntFromHex("b23694f0367f3e621a8458fc24d1dce654be3e2e2c1bacea40cd7a5e7a134540"),
            rBig
        )
        assertEquals(
            bigIntFromHex("1c7ab8e46f91d3f9dbff8c50a37a0d13bbf835ac31c5d0da2dbf1e8a91c10521"),
    ...
    @Test
    fun parseDerSignature_leadingZeroStrippedFromS() {
        val rBytes = hexToBytes("1c7ab8e46f91d3f9dbff8c50a37a0d13bbf835ac31c5d0da2dbf1e8a91c10521")
        val sBytes = hexToBytes("d7fbd22ba32e17ce0f862e83e9c43e768eb3cc7a4ce050f6f71f33f27ce97ba2")
        val der = buildDer(rBytes, byteArrayOf(0x00) + sBytes)
        val (rBig, sBig) = SmartAccountUtils.parseDerSignature(der)
        assertEquals(
            bigIntFromHex("1c7ab8e46f91d3f9dbff8c50a37a0d13bbf835ac31c5d0da2dbf1e8a91c10521"),
            rBig
        ...
        assertEquals(
            bigIntFromHex("d7fbd22ba32e17ce0f862e83e9c43e768eb3cc7a4ce050f6f71f33f27ce97ba2"),
            sBig
        )
    }
    ...
    @Test
    fun parseDerSignature_rEqualsCurveOrder() {
        // n in canonical DER form: the high bit is set, so a 0x00 padding byte is prepended.
        // Stripping must happen before the range check, otherwise the 33-byte size rejects first.
        val nWithPrefix = hexToBytes(
            "00ffffffff00000000ffffffffffffffffbce6faada7179e84f3b9cac2fc632551"
        )
        val s = byteArrayOf(0x01)
        val ex = assertFailsWith<ValidationException.InvalidInput> {
            SmartAccountUtils.parseDerSignature(buildDer(nWithPrefix, s))
        }
        assertTrue(ex.message!!.contains("r component exceeds curve order"))
    }

    @Test
    fun parseDerSignature_sEqualsCurveOrder() {
        val r = byteArrayOf(0x01)
        val nWithPrefix = hexToBytes(
            "00ffffffff00000000ffffffffffffffffbce6faada7179e84f3b9cac2fc632551"
        )
        val ex = assertFailsWith<ValidationException.InvalidInput> {
            SmartAccountUtils.parseDerSignature(buildDer(r, nWithPrefix))
        }
        assertTrue(ex.message!!.contains("s component exceeds curve order"))
    }

    @Test
    fun parseDerSignature_rAboveCurveOrder() {
        // n + 1 is above the valid range [1, n-1]
        val nPlusOneWithPrefix = hexToBytes(
            "00ffffffff00000000ffffffffffffffffbce6faada7179e84f3b9cac2fc632552"
        )
        val s = byteArrayOf(0x01)
        val ex = assertFailsWith<ValidationException.InvalidInput> {
            SmartAccountUtils.parseDerSignature(buildDer(nPlusOneWithPrefix, s))
        }
        assertTrue(ex.message!!.contains("r component exceeds curve order"))
    }
    ...
    fun parseDerSignature_rJustBelowCurveOrder() {
        ...
        val (rBig, sBig) = SmartAccountUtils.parseDerSignature(der)
        assertEquals(
            bigIntFromHex("ffffffff00000000ffffffffffffffffbce6faada7179e84f3b9cac2fc632550"),
            rBig
        )
        assertEquals(BigInteger.ONE, sBig)
    }

    @Test
    fun parseDerSignature_sJustBelowCurveOrder() {
        // S = n - 1 is in range; parseDerSignature enforces only [1, n-1] and leaves
        // the low-S folding to normalizeSignature
        val r = byteArrayOf(0x01)
        val sBytes = hexToBytes(
            "ffffffff00000000ffffffffffffffffbce6faada7179e84f3b9cac2fc632550"
        )
        val der = buildDer(r, byteArrayOf(0x00) + sBytes)
        val (rBig, sBig) = SmartAccountUtils.parseDerSignature(der)
        assertEquals(BigInteger.ONE, rBig)
        assertEquals(
            bigIntFromHex("ffffffff00000000ffffffffffffffffbce6faada7179e84f3b9cac2fc632550"),
            sBig
        )
    ...
    @Test
    fun normalizeSignature_halfOrderPlusOne() {
        // S = halfOrder + 1 is the smallest high-S value; n - S folds it back to halfOrder
        val der = hexToBytes(
        ...
        val compact = SmartAccountUtils.normalizeSignature(der)
        assertEquals(64, compact.size)
        assertContentEquals(expected, compact)
    }

    @Test
    fun normalizeSignature_curveOrderConstantVerification() {
        // n - (n - 1) == 1 holds only if the curve order constant is exact to the bit
        val der = hexToBytes(
            "3045" +
        ...
        assertContentEquals(
            ...
            compact.copyOfRange(32, 64)
        )
    }

    @Test
    fun normalizeSignature_realisticWebAuthnHighS() {
        // Authenticator-shaped signature: 33-byte R and 33-byte S, S above halfOrder
        val der = hexToBytes(
            "3046" +
    ...
    @Test
    fun normalizeSignature_realisticWebAuthnLowS() {
        // Authenticator-shaped signature with S already below halfOrder: passes through unchanged
        val der = hexToBytes(
            "3045" +
    ...
    @Test
    fun normalizeSignature_withLeadingZeros() {
        // 33-byte R and S, each carrying a DER 0x00 padding byte that must be stripped
        val der = hexToBytes(
    ...
    @Test
    fun normalizeSignature_shortComponents() {
        // 4-byte R and 5-byte S are left-padded to 32 bytes each
        val der = hexToBytes("300d02040102030402050506070809")
        val expected = hexToBytes(
    ...
    @Test
    fun extractPublicKey_compressedKeyDoesNotFallThrough() {
        // A compressed prefix is fatal even when a valid attestation object is also supplied
        val compressed = byteArrayOf(0x02) + generatorX
    ...
    @Test
    fun extractPublicKeyFromAuthData_bigEndianCredIdLength() {
        // credentialIdLength = 256 encodes as 0x01 0x00, so the high length byte carries the
        // value and the COSE key sits 256 bytes further into the data
        val authData = buildAuthenticatorData(
    ...
    @Test
    fun extractPublicKeyFromAuthData_rfc6979TestVectorAccepted() {
        // A second known-good P-256 point confirms on-curve validation is not tied to G
        val authData = buildAuthenticatorData(
        ...
        val result = SmartAccountUtils.extractPublicKeyFromAuthenticatorData(authData)
        assertNotNull(result)
        assertContentEquals(rfc6979PublicKeyX, result.copyOfRange(1, 33))
        assertContentEquals(rfc6979PublicKeyY, result.copyOfRange(33, 65))
    }

    @Test
    fun extractPublicKeyFromAuthData_zeroXRejected() {
        // X = 0 is not a valid curve point component
        val authData = buildAuthenticatorData(
    ...
    @Test
    fun extractPublicKeyFromAuthData_zeroYRejected() {
        val authData = buildAuthenticatorData(
    ...
    @Test
    fun extractPublicKeyFromAuthData_xExceedsFieldPrimeRejected() {
        // 32 bytes of 0xFF is 2^256 - 1, which is greater than the field prime p
        val authData = buildAuthenticatorData(
            credentialId = ByteArray(16) { 0x10 },
            x = ByteArray(32) { 0xFF.toByte() },
            y = generatorY
        )
        val ex = assertFailsWith<ValidationException.InvalidInput> {
            SmartAccountUtils.extractPublicKeyFromAuthenticatorData(authData)
    ...
    @Test
    fun extractPublicKeyFromAuthData_yExceedsFieldPrimeRejected() {
        val authData = buildAuthenticatorData(
            credentialId = ByteArray(16) { 0x11 },
            x = generatorX,
            y = ByteArray(32) { 0xFF.toByte() }
        )
        val ex = assertFailsWith<ValidationException.InvalidInput> {
            SmartAccountUtils.extractPublicKeyFromAuthenticatorData(authData)
        }
    ...
    @Test
    fun extractPublicKey_cborDataFallsThroughToAttestationObject() {
        // A 65-byte blob whose first byte is neither 0x04 nor a compressed prefix is not
        // key material, so the attestation object strategy runs
        val cborData = byteArrayOf(0xA3.toByte()) + ByteArray(64) { 0x11 }
        val attestation = buildAttestationObject(generatorX, generatorY)
        val result = SmartAccountUtils.extractPublicKeyFromRegistration(
            publicKey = cborData,
            attestationObject = attestation
     ...
     * [53..54] credentialIdLength (big-endian uint16, taken from credentialId.size)
    ...
    private fun buildAuthenticatorData(
        ...
        val totalSize = 55 + credentialId.size + 10 + 32 + 3 + 32
        ...
        data[53] = ((credentialId.size shr 8) and 0xFF).toByte()
        data[54] = (credentialId.size and 0xFF).toByte()
        credentialId.copyInto(data, 55)

        val coseStart = 55 + credentialId.size
    ...
    /** Parses an unsigned base-16 string into a BigInteger for exact R/S comparisons. */
