...
class SmartAccountKitTest {
    ...
    fun testWalletOperations_createWallet_withMockProvider() = runTest {
        ...
        val mockProvider = CancellableWebAuthnProvider()
    ...
    fun testWalletOperations_createWallet_userCancellation() = runTest {
        val mockProvider = CancellableWebAuthnProvider(shouldCancel = true)
        val config = createTestConfig(webauthnProvider = mockProvider)
        val kit = OZSmartAccountKit.create(config)
    ...
    fun testAddNewPasskeySigner_registersCredentialAndEmitsEvent() = runTest {
        // addNewPasskeySigner registers the passkey, stores the credential and announces it
        // before it ever reaches the contract, so all three are observable even though the
        // RPC boundary here is closed.
        val mockProvider = CancellableWebAuthnProvider()
        val config = createTestConfig(webauthnProvider = mockProvider)
        val storage = InMemoryStorageAdapter()
        val contractId = StrKey.encodeContract(ByteArray(32) { (it + 61).toByte() })
        val kit = OZSmartAccountKit.createWithServer(
            config.copy(storage = storage),
            buildNoRpcMockServer()
        )

        kit.setConnectedState("existing-credential", contractId)
        assertTrue(kit.isConnected)
        ...
        // The mock server rejects every request, so the call fails at the on-chain step.
        // Asserting that it is not a validation failure is what keeps the assertions below
        // meaningful: a malformed fixture would otherwise abort before any of them applies.
        ...
        assertFalse(
            ...
            "The flow must reach the on-chain step rather than fail local validation, got: $failure"
        )

        val emittedCredential = assertNotNull(
            credentialCreatedEvent,
            "CredentialCreated must be emitted before the on-chain step"
        ).credential
        ...
        assertEquals(contractId, emittedCredential.contractId)
        assertEquals(CredentialDeploymentStatus.PENDING, emittedCredential.deploymentStatus)

        // The announced credential is the one that reached storage, and it is still pending
        // because the on-chain step never completed.
        val newCredential = assertNotNull(
            storage.get(emittedCredential.credentialId),
            "The announced credential must be persisted"
        )
        assertTrue(
            emittedCredential.publicKey.contentEquals(newCredential.publicKey),
            "The stored public key must be the one the authenticator produced"
        )
        assertEquals(contractId, newCredential.contractId)
        assertEquals(CredentialDeploymentStatus.PENDING, newCredential.deploymentStatus)
    }
    ...
    // MARK: - 6b. AddPasskeySignerResult Tests

    private fun addPasskeySignerResult(
        credentialId: String = "Y3JlZC1vbmU",
        publicKeyByte: Byte = 0x11,
        transactionResult: TransactionResult = TransactionResult(success = true, hash = "hash-1", ledger = 42u)
    ) = AddPasskeySignerResult(
        credentialId = credentialId,
        publicKey = ByteArray(65) { if (it == 0) SmartAccountConstants.UNCOMPRESSED_PUBKEY_PREFIX else publicKeyByte },
        transactionResult = transactionResult
    )

    @Test
    fun testAddPasskeySignerResult_sameFieldValues_areEqualAndShareHashCode() {
        val result1 = addPasskeySignerResult()
        val result2 = addPasskeySignerResult()

        // The public key is held as a ByteArray, so equality must compare content and not
        // array identity.
        assertFalse(result1.publicKey === result2.publicKey)
        assertEquals(result1, result2)
        assertEquals(result1.hashCode(), result2.hashCode(), "Equal results must have equal hashCode")

        assertEquals(result1, result1, "A result must equal itself")
        assertEquals("Y3JlZC1vbmU", result1.credentialId)
        assertEquals(65, result1.publicKey.size)
        assertEquals("hash-1", result1.transactionResult.hash)
    }

    @Test
    fun testAddPasskeySignerResult_differentCredentialId_areNotEqual() {
        assertFalse(
            addPasskeySignerResult(credentialId = "Y3JlZC1vbmU") ==
                addPasskeySignerResult(credentialId = "Y3JlZC10d28"),
            "Results for different credentials must not be equal"
        )
    }

    @Test
    fun testAddPasskeySignerResult_differentPublicKey_areNotEqual() {
        assertFalse(
            addPasskeySignerResult(publicKeyByte = 0x11) == addPasskeySignerResult(publicKeyByte = 0x22),
            "Results carrying different public keys must not be equal"
        )
    }

    @Test
    fun testAddPasskeySignerResult_differentTransactionResult_areNotEqual() {
        assertFalse(
            addPasskeySignerResult(
                transactionResult = TransactionResult(success = true, hash = "hash-1")
            ) == addPasskeySignerResult(
                transactionResult = TransactionResult(success = false, error = "failed")
            ),
            "Results with different on-chain outcomes must not be equal"
        )
    }

    @Test
    fun testAddPasskeySignerResult_otherTypeOrNull_isNotEqual() {
        val result = addPasskeySignerResult()

        assertFalse(result.equals(null))
        assertFalse(
            result.equals(TransactionResult(success = true, hash = "hash-1", ledger = 42u)),
            "A result must not equal a value of another type"
        )
    }
...
// MARK: - Cancellable WebAuthn Provider

/**
 * WebAuthn provider stub whose ceremonies can be made to cancel or fail on demand.
 * Registration yields a random credential ID and a deterministic uncompressed public key.
 */
private class CancellableWebAuthnProvider(
