 ...
 * ```
 *
 * @property line The asset or liquidity pool for the trustline
 * @property limit The limit of the trustline (defaults to MAX_LIMIT)
 */
data class ChangeTrustOperation private constructor(
    val line: ChangeTrustAsset,
    val limit: String
) : Operation() {

    // Convenience constructor for Asset
    constructor(asset: Asset, limit: String = MAX_LIMIT) : this(ChangeTrustAsset(asset), limit) {
        require(asset !is AssetTypeNative) {
            "ChangeTrust cannot be used with the native asset"
        }
    }

    // Convenience constructor for LiquidityPool
    constructor(liquidityPool: LiquidityPool, limit: String = MAX_LIMIT) : this(ChangeTrustAsset(liquidityPool), limit)
    ...
    /**
     * Legacy property for backward compatibility. Returns the asset if this is an asset trustline,
     * throws exception if this is a liquidity pool trustline.
     * @throws UnsupportedOperationException if this is a liquidity pool trustline
     */
    val asset: Asset
        get() = when (line) {
            is ChangeTrustAsset.Wrapper -> line.asset
            is ChangeTrustAsset.LiquidityPoolShare -> throw UnsupportedOperationException(
                "This is a liquidity pool trustline. Use 'line' property to access the ChangeTrustAsset."
            )
        }
    ...
    companion object {
        ...
        fun fromXdr(op: ChangeTrustOpXdr): ChangeTrustOperation {
            val changeTrustAsset = ChangeTrustAsset.fromXdr(op.line)
            val limit = fromXdrAmount(op.limit.value)
            return ChangeTrustOperation(changeTrustAsset, limit)
        }
    }
}
