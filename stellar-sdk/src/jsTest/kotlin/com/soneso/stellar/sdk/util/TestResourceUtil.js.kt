...
actual object TestResourceUtil {
    ...
    actual fun readFixtureBytes(filename: String): ByteArray {
        return if (isNodeJs()) {
            readFixtureBytesNode(filename)
        } else {
            readFixtureBytesBrowser(filename)
        }
    }
    ...
    private fun readFixtureBytesNode(filename: String): ByteArray {
        return try {
            val fs = js("eval('require')('fs')")
            val path = js("eval('require')('path')")
            ...
            val dirname = js("__dirname") as String
            val paths = arrayOf(
                path.resolve(dirname, "fixtures", filename) as String,
            ...
            throw IllegalArgumentException(
                "Fixture file not found in any expected location: '$filename' " +
                "(searched from __dirname=$dirname)"
            )
        } catch (e: Throwable) {
    ...
    private fun readFixtureBytesBrowser(filename: String): ByteArray {
        val url = "/fixtures/$filename"
        ...
        return try {
            ...
            if (status != 200) {
                throw IllegalArgumentException(
                    "Failed to fetch fixture file '$filename' from Karma server: HTTP $status (url: $url)"
                )
            }
            ...
            throw IllegalArgumentException(
                "Failed to read fixture file '$filename' in browser: ${e.message}", e
            )
        }
    }
