...
Because smart accounts are contracts, they cannot use Stellar's built-in Ed25519 signature verification. A separate verifier contract deployed on-chain validates secp256r1 signatures. When a user authenticates with their passkey, the signature is submitted to the smart account contract, which forwards it to the verifier contract. If the signature is valid and all policies are satisfied, the transaction executes.
...
// Query rules of a specific type
val rules = kit.contextRuleManager.getContextRules(ContextRuleType.Default)
