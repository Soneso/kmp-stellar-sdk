...
class DemoTokenService(
    ...
    suspend fun ensureTokenAndMint(recipientContractId: String): DemoTokenResult {
        ...
        // Pre-compute the expected contract address with Address.deriveContractId before
        // deploying. The deployed address is checked against it after first deployment.
        ...
        if (!alreadyExisted) {
            ...
            // Verify determinism: the address the network assigned must match the one
            // Address.deriveContractId predicted from the same deployer, salt and network.
            // A mismatch means the pre-derived address the rest of the demo uses is wrong.
    ...
    companion object {
        ...
        suspend fun deriveTokenContractAddress(networkPassphrase: String): String {
            val adminKeyPair = deriveTokenAdminKeyPair()
            val salt = deriveTokenSalt()
            return Address.deriveContractId(
                deployer = Address(adminKeyPair.getAccountId()),
                salt = salt,
                network = Network(networkPassphrase)
            )
        }
